/* ═══ page-entry.js — الإدخال اليدوي (محدّث ومتوافق مع المواد المتعددة) ═══ */
...
registerPage('entry', {
  enter: function() {
    ...
    if (ADM.teachers && ADM.teachers.length) {
      entRenderTeachers();
    } else {
...
function entSelectTeacher(code) {
  ...
  for (var i = 0; i < ADM.teachers.length; i++) {
    if (ADM.teachers[i].code === code) { ENT.teacher = ADM.teachers[i]; break; }
  }
  ...
  if (ENT.teacher.subjects && ENT.teacher.subjects.length > 0) {
    ENT.teacher.subjects.forEach(function(sub) {
      h += '<button class="btn ok" style="width:auto;flex:1;min-width:140px" onclick="entSelectSubject(\'' + escA(sub.name) + '\')">📘 ' + esc(sub.name) + '</button>';
    });
  } else {
    h += '<div class="hint">لا توجد مواد مسجلة لهذا المعلم</div>';
  }
  h += '</div></div>';
...
function entSelectSubject(subName) {
  ENT.subject = subName;
  ENT.cls = null; ENT.names = []; ENT.rows = [];
  
  // العثور على المادة المحددة لجلب صفوفها فقط
  var targetSub = null;
  if (ENT.teacher && ENT.teacher.subjects) {
    for (var i = 0; i < ENT.teacher.subjects.length; i++) {
      if (ENT.teacher.subjects[i].name === subName) {
        targetSub = ENT.teacher.subjects[i];
        break;
      }
    }
  }
  ...
  if (targetSub && targetSub.classes && targetSub.classes.length > 0) {
    targetSub.classes.forEach(function(c) {
      var parsed = parseCls(c); // استخدام دالة parseCls من core.js
      h += '<button class="btn" style="width:auto;flex:1;min-width:120px" onclick="entSelectClass(\'' + escA(parsed.grade) + '\',\'' + escA(parsed.section) + '\')"> 🏫 ' + esc(c) + '</button>';
    });
  } else {
    h += '<div class="hint">لا توجد صفوف مسجلة لهذه المادة</div>';
  }
  h += '</div></div>';
  ...
  // تحديد تلقائي إذا كان هناك صف واحد فقط
  if (targetSub && targetSub.classes && targetSub.classes.length === 1) {
    var parsed = parseCls(targetSub.classes[0]);
    entSelectClass(parsed.grade, parsed.section);
  }
}
...
function entRenderTable() {
  ...
  var thead = '<thead><tr>'
    ...
    + '<th class="enter" style="position:sticky;top:0;z-index:10;background:#1D4ED8;color:#fff;min-width:50px">ت١</th>'
    + '<th class="enter" style="position:sticky;top:0;z-index:10;background:#1D4ED8;color:#fff;min-width:50px">ت٢</th>'
    + '<th class="enter" style="position:sticky;top:0;z-index:10;background:#1D4ED8;color:#fff;min-width:50px">ك١</th>'
...
function entSave() {
  ...
  var rows = ENT.rows.map(function(r) {
    return {
      name: r.name, subject: ENT.subject,
  ...
  toast('⏳ حفظ...', '');
  // إضافة subject صراحةً للـ payload لضمان تطابقه مع adminSubmit
  var payload = entPayload({ action: 'adminSubmit', rows: rows, subject: ENT.subject });
...
function entPrintHTML() {
  ...
  /* ✅ تحويل العام الدراسي إلى أرقام عربية باستخدام AR من core.js */
  var studyYearAr = String(studyYear).replace(/\d/g, function(d) { return AR[+d]; });
  ...
  pages.forEach(function(pageRows, pageIdx) {
    ...
    h += '<th style="' + th + ';width:50px">ت١</th><th style="' + th + ';width:50px">ت٢</th><th style="' + th + ';width:50px">ك١</th>';
    h += '<th style="' + thC + '">معدل ف١</th>';
    h += '<th style="' + th + '">نصف السنة</th>';
    h += '<th style="' + th + ';width:50px">آذار</th><th style="' + th + ';width:50px">نيسان</th>';
    ...
    pageRows.forEach(function(r, i) {
      ...
      function cell(v, extra, style) {
        var cellStyle = style || td;
        if (v == null || v === '') {
          return '<td style="' + cellStyle + (extra || '') + '"></td>';
        }
        return '<td style="' + cellStyle + (extra || '') + '">' + arNum(v) + '</td>';
      }
